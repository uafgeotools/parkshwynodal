...
def load_waveform(sta, arrive_time, spec_window=120, component="Z"):
	...
	Args:
		...
		arrive_time (float): Acoustic wave arrival time in 
			seconds since the epoch.
		spec_window (int): Time window in seconds to trim the waveform data, 
			default is 120 seconds.

	Returns:
		tuple: A tuple containing the waveform data, sampling frequency, time 
			data correlating to waveform, and title
	"""
	...
	tr = client.get_waveforms(
		"ZE", str(sta), "*", f'DP{component}', starttime, endtime)
	tr = tr[0]
	data = tr.data
	sample_rate = int(tr.stats.sampling_rate)
	title = f'{tr.stats.network}.{tr.stats.station}.{tr.stats.location}.'
	title += f'{tr.stats.channel} − starting {tr.stats["starttime"]}'
	t_wf = tr.times()
