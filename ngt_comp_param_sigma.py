...
axs[0, 1].set_xlim(50, 2500)
axs[0, 1].set_ylim(50, 2500)
...
axs[1, 0].set_ylabel(str(len(diff_speed)-1) + '/' + str(len(diff_speed)) + ' samples')
...
axs[1, 0].set_title('Median Velocity Difference (m/s): {:.2f} ± {:.2f}'.format(np.median(diff_speed), rms_speed), fontsize=14)
...
axs[1, 1].set_ylabel(str(len(diff_dist)) + '/' + str(len(diff_dist)) + ' samples')
...
axs[1, 1].set_title('Median Distance Difference (m): {:.2f} ± {:.2f}'.format(np.median(diff_dist), rms_dist), fontsize=14)
