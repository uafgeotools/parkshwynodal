        ...
        for line in file.readlines():
            ...
            for ii, ss in enumerate(sta_loc):
                if float(ss) == float(sta) and int(flight_id[ii]) == int(flight_num):
                    ...
                    angles.append(np.cos(np.arctan( float(dist_m[closest_index]) / (float(alt[closest_index]) - elev))))
                    if error_bar:
...
#sc0 = axs[0, 0].scatter(inverse_speeds, fr_speeds, c='k', cmap='cividis', s=15, zorder=2)
...
axs[0, 0].set_title("Speed (m/s)", fontsize=10)
