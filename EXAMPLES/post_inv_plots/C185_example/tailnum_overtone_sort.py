...
file = pd.read_csv(
    f'{repo_path}/output/Seppi_2025_results/NGT_flight_param_inv_DB.txt', 
    sep=",")

flight_nums = file['Flight_Number']
freq_peaks = file['Meas_Source_Frequency_Array']

flight_num_dict = {'10512184': ['527958214', '529754214', '529970458', 
                                '530342801', '530489496', '530681886', 
                                '530893864', '531043310', '531236830', 
                                '531254054', '533770152', '533950817', 
                                '534899218'], 
                    '10572742': ['528502194', '528518474', '528510459', 
                                '528711629', '528698927', '529409728', 
                                '529416700', '530144820', '530501643', 
                                '530672521', '530695031', '530706111', 
                                '530842926', '531272879', '531417015', 
                                '531424585', '531583949', '531605202', 
                                '531901801', '531774192', '533421550']}
...
for tail_num, f_nums in flight_num_dict.items():
    ...
    for f_num in f_nums:
        #extract all the index values in flight_nums corresponding to f_num
        indices = [i for i, fn in enumerate(flight_nums) if int(fn) == int(f_num)]
...
print('Crossings for 10572742:', len(all_med['10572742']))
print('Crossings for 10512184:', len(all_med['10512184']))
...
for tail_num, peaks in peaks_dict.items():
    ...
    ax1.hist(peaks, bins=270, color=color, alpha=0.8, label=tail_num, 
             zorder = 10)  
...
ax2.tick_params(left=False, right=False, labelleft=False, labelbottom=True,
                bottom=True)
...
x_label = []
for g in range(0,14):
    ax1.axvline(x= (1 + g) * del_f_t1, color = [0.0, 0.5, 1.0], ls = '--', 
                zorder=0, linewidth=1)
    if g != 13:
        x_label.append((1 + g) * del_f_t1)
    ax1.axvline(x= (1 + g) * del_f_t2_1, color = [1.0, 0.5, 0.0], ls = '--', 
                zorder=0, linewidth=1)
    ax1.axvline(x= (1 + g) * del_f_t2_2, color = [1.0, 0.5, 0.0], ls = '--', 
                zorder=0, linewidth=1)
...
ax2.axvline(x=del_f_t1, color = [0.0, 0.5, 1.0], ls = '--', 
            zorder=0, linewidth=1)
ax2.axvline(x=del_f_t2_1, color =   [1.0, 0.5, 0.0], ls = '--', 
            zorder=0, linewidth=1)
ax2.axvline(x=del_f_t2_2, color =  [1.0, 0.5, 0.0], ls = '--', 
            zorder=0, linewidth=1)
