...
This example is for a small plane passing by the AVO KENI infrasound array.
'''
...
# read in infrasound data from the AVO KENI array
from obspy.clients.fdsn import Client
client = Client("IRIS")
T0 = UTCDateTime(2026, 6, 22, 0, 58, 30)
print(f"Running for {T0.strftime('%Y-%m-%d %H:%M:%S')}")

DUR = 120
st = client.get_waveforms('AV', "KENI", '01', 'HDF',
                          starttime = T0, endtime = T0+120)
tr = st[0]
tr.detrend(type='linear')
tr.taper(max_percentage=.01)
tr.filter('bandpass', freqmin=1, freqmax=50, corners=2, zerophase=True)

data = tr.data
t_wf = tr.times()
sample_rate = tr.stats.sampling_rate
arrive_time = T0.timestamp
title = f"{tr.id} - {T0.strftime('%Y-%m-%d %H:%M:%S')}"
